...
def read_img(img_path):
    im = Image.open(img_path).convert('L') # 转换为灰度图像
...
def get_imgs_path(dir_path):
    ...
    for dir in list_dir:
        ...
        for img_name in dir_img_name_list:
            ...
            if not strs[0] in SEX:
                SEX.append(strs[0])
            if not strs[1] in MOOD:
                MOOD.append(strs[1])
...
def get_imgs(img_paths):
    ...
    for img_path in img_paths:
        ...
        imgs.append(img_val)
        face_sex = np.ones(len(SEX)) / 10
        face_mood = np.ones(len(MOOD)) / 10

        strs = img_path.split('/')[-1].split('_')
        face_sex[SEX.index(strs[0])] = 0.9
        face_mood[MOOD.index(strs[1])] = 0.9

        sex.append(face_sex)
        mood.append(face_mood)

    pass

    return_val = {
        'imgs': np.array(imgs),
        'face_sex': np.array(sex),
        'face_mood': np.array(mood),
    }
    return return_val
...
def main():
    ...
    train_data = get_imgs(img_name_list[:60])
    test_data = get_imgs(img_name_list[60:])
