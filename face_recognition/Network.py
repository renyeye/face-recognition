...
class Network(object):
    ...
    def test(self,input_data, correct_result):
        error_times = 0
        diff_val = 0
        predict_index_list = [] #加
        for input_val, out_put in zip(input_data, correct_result):
            self.layer[0].layer_predict(input_val)
            for j in range(1, self.layer_num):
                # print('j = ' + str(j))
                self.layer[j].layer_predict(self.layer[j - 1].outputs)
            ...
            if not np.argmax(predict_val) == np.argmax(out_put):
                error_times += 1
            predict_index_list.append(np.argmax(predict_val)) # 加
            diff_val += np.sum(np.square(predict_val - out_put))
        pass
        return {'error_times':error_times,"diff_val":diff_val,"predict_index_list":predict_index_list}#添加返回值predict_val
pass